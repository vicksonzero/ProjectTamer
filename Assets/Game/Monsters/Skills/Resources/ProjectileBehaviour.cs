...
public class ProjectileBehaviour : PhotonView
{
    ...
    void OnPhotonInstantiate()
    {
        ...
        this.damages = (float[])this.photonView.instantiationData[(int)ProjectileParameters.damages];
        //this.debuffs = (string[])this.photonView.instantiationData[(int)ProjectileParameters.debuffs];
    }
    ...
    void OnCollisionEnter(Collision coln)
    {
        if (coln.collider.transform == this.target)
        {
            ...
            PhotonNetwork.Instantiate(this.hitEffectPrefab.name, this.transform.position, Quaternion.identity,0);
            // bye!
